...
interface RectangleDrawingProps {
  activeTool: string;
...
export default function EnhancedRectangleDrawing({
  activeTool,
  onRectangleCreate,
  onMeasurementUpdate,
  selectedLayerId,
  layers,
}: RectangleDrawingProps) {
    ...
    (event: any) => {
      ...
      if (point) {
        ...
        if (previewRef.current) {
          ...
          previewRef.current.position.set(centerX, 0.01, centerZ);
          previewRef.current.scale.set(width, 1, height);
        }
      }
    },
  ...
  // Clean up preview mesh on unmount
  useEffect(() => {
    return () => {
      if (previewMesh) {
        scene.remove(previewMesh);
    ...
    <>
      ...
      {isDrawing && startPoint && currentPoint && (
        ...
        >
          ...
          />
          <meshBasicMaterial
            color={isSnapping ? 0x10b981 : 0x3b82f6}
            transparent
            opacity={0.6}
            side={THREE.DoubleSide}
          />
      ...
      {/* Snap Indicators */}
      {isSnapping && currentPoint && (
        <mesh position={[currentPoint.x, 0.02, currentPoint.z]}>
          <sphereGeometry args={[0.05]} />
          <meshBasicMaterial color={0x10b981} />
        </mesh>
      )}

      {/* Snap Points Visualization (for debugging) */}
      {snapPoints.length > 0 && (
        <group>
          {snapPoints.map((point, index) => (
            <mesh key={index} position={[point.x, 0.005, point.z]}>
              <sphereGeometry args={[0.02]} />
              <meshBasicMaterial color={0xef4444} transparent opacity={0.3} />
            </mesh>
          ))}
        </group>
      )}
